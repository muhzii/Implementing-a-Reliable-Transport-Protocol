...
int seq_A=0; // packet currently being sent
int ack_B=1; // last acknowledged packet
_Bool can_send = 1;
struct pkt pkt2send;
  ...
  struct msg message;
{
   ...
   pkt2send = make_pkt(seq_A, message);
   printf("  A: Sending: %.*s, chksum: %d\n", 20, message.data, pkt2send.checksum);
	tolayer3(0, pkt2send); // send packet to the network
	starttimer(0, TIME_OUT); // start timer
}
  ...
  struct pkt packet;
{
	...
	if(corrupt) {
		tolayer3(1, mack_ack(ack_B)); // send last ack number
